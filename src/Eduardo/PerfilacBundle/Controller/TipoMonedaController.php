...
use Eduardo\PerfilacBundle\Entity\TipoMoneda;
...
class TipoMonedaController extends Controller {
    ...
    public function incluirAction($accion) {
        ...
        $msg = "";
        if ($accion == 'ini')
            return $this->render('PerfilacBundle:nomencladores:incluirTipoMoneda.html.twig', array('msg' => $msg));
    ...
    public function listarAction() {
        ...
        $listTipoMonedas = $paginator->paginate($this->listar())->getResult();

        $msg = "";
        return $this->render('PerfilacBundle:nomencladores:listarTipoMoneda.html.twig', array('tiposMoneda' => $listTipoMonedas, 'msg' => $msg));
    }
    
    public function modificarAction($id) {
        
        $msg = "";
        $tipoMoneda = $this->obtener($id);  
        if($tipoMoneda != NULL)
          return $this->render('PerfilacBundle:nomencladores:modificarTipoMoneda.html.twig', array('tipoMoneda' => $tipoMoneda, 'msg' => $msg));
        else
        {
          #en caso que no exista un objeto con tal id redirecciono para el listar
          return  new RedirectResponse($this->generateUrl('tipoMonedaListar')); 
        }
    }
    ...
    private function salvar($p_nombre) {
        ...
        $tipoMoneda = new TipoMoneda($p_nombre);        
        $em->persist($tipoMoneda);
        $em->flush();
    }

    private function listar() {
        $repository = $this->getDoctrine()->getRepository('PerfilacBundle:TipoMoneda');
        $tiposMonedas = $repository->findBy(array(), array('nombre' => 'ASC'));
        return $tiposMonedas;
    }
    
    private function obtener($id) {
            $repository = $this->getDoctrine()->getRepository('PerfilacBundle:TipoMoneda');
            $tipoMoneda = $repository->find($id);
            return $tipoMoneda;
        }
    ...
    private function actualizar($p_id,$p_nombre) {
            $em = $this->getDoctrine()->getManager();    
            $repository = $this->getDoctrine()->getRepository('PerfilacBundle:TipoMoneda');
            $tipoMoneda = $repository->find($p_id);
            $tipoMoneda->setNombre($p_nombre);
            $em->flush();
        }
